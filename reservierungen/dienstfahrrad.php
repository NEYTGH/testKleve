...
if($db_conn){
	...
	$stmt=$db_conn->query("SELECT id, name, reichweite FROM companybicycle_pool ORDER BY id");
	$dienstfahrrad_typ=$stmt->fetchALL(PDO::FETCH_ASSOC);
    
    // Alle Reservierungen für die nächsten 14 Tage abrufen
    $stmt = $db_conn->prepare("
        SELECT r.id, r.datum, r.startZeit, r.endZeit, r.dienstfahrrad, r.zweck, 
               m.nachname, m.vorname, c.name as bike_name
...
if($_SERVER['REQUEST_METHOD']==='POST'){
	...
	$dienstfahrrad = filter_input (INPUT_POST, 'dienstfahrrad', FILTER_SANITIZE_STRING);
    ...
    if (empty($dienstfahrrad)) {
        $errors[] = "Bitte wählen Sie ein Dienstfahrrad aus.";
    }
    ...
    if (empty($errors) && $db_conn) {
        try {
            // Prüfen, ob Dienstfahrrad für den Zeitraum verfügbar ist
            $stmt = $db_conn->prepare("
                SELECT COUNT(*) FROM companybicycle_reservations 
                WHERE datum = :datum AND dienstfahrrad = :dienstfahrrad AND (
            ...
            $stmt->bindParam(':dienstfahrrad', $dienstfahrrad);
            $stmt->execute();

            $count = $stmt->fetchColumn();

            if ($count > 0) {
                // Dienstfahrrad ist bereits reserviert
                $errors[] = "Das Dienstfahrrad ist im gewählten Zeitraum leider nicht verfügbar.";
            } else {
                // Dienstfahrrad ist verfügbar, Reservierung eintragen
                // Generiere eine UUID für das Storno-Feld
                $storno = uniqid('', true);
                
                $stmt = $db_conn->prepare("
                    INSERT INTO companybicycle_reservations (datum, startZeit, endZeit, benutzer_id, dienstfahrrad, zweck, bemerkung, storno)
                    VALUES (:datum, :startZeit, :endZeit, :benutzerId, :dienstfahrrad, :zweck, :bemerkung, :storno)
                ");
                ...
                // Fahrraddetails abrufen
                $stmt = $db_conn->prepare("SELECT name FROM companybicycle_pool WHERE id = :id");
                $stmt->bindParam(':id', $dienstfahrrad);
                $stmt->execute();
                $fahrradName = $stmt->fetchColumn();
                ...
                $reservationData = [
                    ...
                    'dienstfahrrad' => $fahrradName,
                    'zweck' => $zweck,
                    'bemerkung' => $bemerkung
                ];

                $_SESSION['success_message'] = "Das Dienstfahrrad wurde erfolgreich reserviert.";
            ...
            error_log("Fehler bei der Dienstfahrradreservierung: " . $e->getMessage());
...
// Reservierungen nach Fahrrad gruppieren für die Übersichtstabelle
$reservationsByBikeDate = [];
$dateRange = [];
...
foreach ($period as $date) {
    ...
    foreach ($dienstfahrrad_typ as $bike) {
        $reservationsByBikeDate[$bike['id']][$dateStr] = [];
    }
}

// Reservierungen in die Struktur einfügen
foreach ($reservations as $reservation) {
    $bikeId = $reservation['dienstfahrrad'];
    $dateStr = $reservation['datum'];
    
    if (isset($reservationsByBikeDate[$bikeId][$dateStr])) {
        $reservationsByBikeDate[$bikeId][$dateStr][] = [
            'id' => $reservation['id'],
...
// Farben für die Dienstfahrräder
$bikeColors = [
    '1' => '#4299e1', // Blau
    '2' => '#48bb78', // Grün
    '3' => '#ed8936', // Orange
...
<head>
    ...
    <title>Kreis Kleve - Dienstfahrrad-Reservierung</title>
...
<body>
    <div class="container">
        <header>
            ...
            <nav>
                <ul>
                    ...
                    <li><a href="dienstfahrrad.php" class="active">Dienstfahrrad-Reservierung</a></li>
        ...
        <main>
            <h2>Dienstfahrrad-Reservierung</h2>
            ...
            <div id="success-message" class="message success">
                ...
                <div class="reservation-details">
					...
					<div class="detail-item">
						...
						<div class="detail-item">
							<span class="label">Dienstfahrrad:</span>
							<span><?php echo htmlspecialchars($reservationData['dienstfahrrad']); ?></span>
						</div>
                        <div class="detail-item">
            ...
            <form id="reservierungForm" class="reservation-form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
				...
				<div class="form-group">
					<label for="dienstfahrrad">Dienstfahrrad*</label>
					<select id="dienstfahrrad" name="dienstfahrrad" required>
						<option value="">Bitte auswählen</option>
						<?php foreach ($dienstfahrrad_typ as $typ): ?>
            ...
            <div class="timeline-container">
                ...
                <div class="timeline">
                    <table class="timeline-table">
                        ...
                        <tbody>
                            <?php foreach ($dienstfahrrad_typ as $bike): ?>
                            <tr>
                                <td>
                                    <?= htmlspecialchars($bike['name']) ?>
                                    <div class="vehicle-reichweite"><?= htmlspecialchars($bike['reichweite']) ?> km</div>
                                </td>
                                <?php 
                                foreach ($dateRange as $date): 
                                    $dateObj = new DateTime($date);
                                ...
                                <td class="<?= $class ?>">
                                    <?php
                                    if (isset($reservationsByBikeDate[$bike['id']][$date])) {
                                        foreach ($reservationsByBikeDate[$bike['id']][$date] as $reservation) {
                                            echo '<div class="reservation-block" style="background-color: ' . 
                                                 ($bikeColors[$bike['id']] ?? '#4299e1') . '">';
